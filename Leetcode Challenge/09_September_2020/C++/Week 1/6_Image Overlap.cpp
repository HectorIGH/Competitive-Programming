    ...
    int largestOverlap(vector<vector<int>>& img1, vector<vector<int>>& img2) {
        ...
        for(int y_shift = 0; y_shift < n; y_shift++) {
            for(int x_shift = 0; x_shift < n; x_shift++) {
                max_overlaps = max(max_overlaps, helper(img1, img2, x_shift, y_shift, n));
                max_overlaps = max(max_overlaps, helper(img2, img1, x_shift, y_shift, n));
            }
        }
        return max_overlaps;
    }
    
    int helper(vector<vector<int>>& A, vector<vector<int>>& B, int x_shift, int y_shift, int n) {
        int left_count = 0;
        int right_count = 0;
