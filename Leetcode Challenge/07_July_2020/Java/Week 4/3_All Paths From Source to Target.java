...
class Solution {
    ...
    public void dfs(int[][] graph, int source, int destiny, List<Integer> path, List<List<Integer>> ans) {
        if(source == destiny) {
            ans.add(path);
        }
        path.add(source);
        for(int v : graph[source]) {
            ...
            dfs(graph, v, destiny, copyPath, ans);
        }
    }
}